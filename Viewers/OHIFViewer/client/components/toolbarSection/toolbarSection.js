    ...
    instance.data.studies[0].seriesList.forEach((serie) => {
        if(serie.modality=="SR" || serie.modality=="DOC") {
            $("#bodyModalDocuments").append(
                ...
                "<div class='col-lg-3 '>"+
 "<button style='background-color: darkorange;color: black' class='btn btn-xs enlaceDocumento col-lg-12' data-url='"+currentServer.docUriRoot+"/IHERetrieveDocument?requestType=DOCUMENT&documentUID=" + serie.instances[0].sopInstanceUid + "&preferredContentType=application/pdf'>"+TAPi18n.__('open')+"</button> "+                "</div>"+
                "</div>"
            );
            numeroDocumentos++;
        }
